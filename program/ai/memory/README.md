...
- **Replay:** `program/utils/replay_db/replay.json` (path from `get_current_dir()` in `utils/path_config.py`).
- **RAG:** `program/utils/memory_db/` (default `get_memory_db_path()`).
...
See **`program/tools/README_RAG_tools.md`** (`rag_manage.py`, `rag_test_compare.py`). Run commands with **`program`** as the working directory so imports resolve.

## View memories

From `program` (with `PYTHONPATH` including `program` if needed):
